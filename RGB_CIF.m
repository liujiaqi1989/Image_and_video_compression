...
for i=1:3
    tim1(:,:,i)=resample(tim(:,:,i),1,2);
    tim2(:,:,i)=tim1(:,:,i)';
    tim3(:,:,i)=resample(tim2(:,:,i),1,2);
    tim4(:,:,i)=tim3(:,:,i)';
end
for i=1:3
   tim5(:,:,i)=resample(tim4(:,:,i),2,1);
    tim6(:,:,i)=tim5(:,:,i)';
    tim7(:,:,i)=resample(tim6(:,:,i),2,1);
    tim8(:,:,i)=tim7(:,:,i)';  
end
[height,width,dim]=size(tim);
bitrate=(1/4*height*width+1/4*(height*width)+1/4*(height*width))*8/(width*height);
