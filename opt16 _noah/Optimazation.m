...
for i = 1 : NumImage
    ...
    ImageSequence(i).Im_Original_small=double(imread(ImageSequence(i).Image_loc));
    ImageSequence(i).Im_Original = padarray(ImageSequence(i).Im_Original_small,[200,0],'replicate','post');
    ...
    ImageSequence(i).Im_Reconstruct_small= ImageSequence(i).Im_Original_small;
end
[h,w,d] = size(ImageSequence(1).Im_ict);
PSNR=zeros(1,5);
PSNR_mean=zeros(1,6);
PSNR_still=zeros(1,6);
BR=zeros(1,5);
BR_mean=zeros(1,6);
BR_still=zeros(1,6);
MV=zeros(h*w/256,2);
MV_m_re=zeros(h/16,w/16,2);
...
for QP=40:-4:20
    ...
    for j = 1 :NumImage
        if j==1
[im_block,~, ~]=block_splitter(ImageSequence(j).Im_ict,16);
...
ImageSequence(j).Im_Re =IntraDecode_hevc(bytestream,BinaryTree,L,db,h,w,d,QP,M);
ImageSequence(j).Im_Reconstruct=ictYCbCr2RGB(ImageSequence(j).Im_Re);
ImageSequence(j).Im_Reconstruct_small=ImageSequence(j).Im_Reconstruct(1:h-200,1:w,1:3);
mse1=calcMSE(h,w,d,ImageSequence(j).Im_Original_small/256,ImageSequence(j).Im_Reconstruct_small/256);
PSNR(1,j)=calcPSNR(mse1,1); 
BR_still(1,k)=BR(1,j);
PSNR_still(1,k)=PSNR(1,j);
        else
            [err_im, MV_m] = MotionEstimation_h_haf(ImageSequence(j).Im_ict,ImageSequence(j-1).Im_Re,16);
            [zr1,L1]=IntraEncode_noHuffman_hevc(err_im,QP,M);
            ...
            %bytestream2=enc_huffman( MV(:)+9 , BinCode2, Codelengths2);
            bytestream2=enc_huffman( MV(:)+18 , BinCode2, Codelengths2);
            L2=length(MV(:));
            BR(1,j)=8*(length(bytestream1)+length(bytestream2))/(h*w);
            err_re=IntraDecode_hevc(bytestream1,BinaryTree1,L1,db,h,w,d,QP,M);%decode mv and get the new MCP
            %MV_re_v=dec_huffman(bytestream2,BinaryTree2,L2)-9;
            MV_re_v=dec_huffman(bytestream2,BinaryTree2,L2)-18;
            MV_re=reshape(MV_re_v,h*w/256,2);
            MV_m_re(:,:,1)=reshape(MV_re(:,1),h/16,w/16);
            MV_m_re(:,:,2)=reshape(MV_re(:,2),h/16,w/16);
            image_pred=MCP_haf(ImageSequence(j-1).Im_Re,MV_m_re,16);
            ImageSequence(j).Im_Re=err_re+image_pred;
            ImageSequence(j).Im_Reconstruct = ictYCbCr2RGB(ImageSequence(j).Im_Re);
           ImageSequence(j).Im_Reconstruct_small=ImageSequence(j).Im_Reconstruct(1:h-200,1:w,1:3);
            mse_r=calcMSE(h,w,d,ImageSequence(j).Im_Original_small/256,ImageSequence(j).Im_Reconstruct_small/256);
