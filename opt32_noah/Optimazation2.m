...
BR_mean=zeros(1,6);
BR_still=zeros(1,6);
MV=zeros(h*w/(32*32),2);
MV_m_re=zeros(h/32,w/32,2);
tic
k=1;
for QP=40:-4:20
    ...
    for j = 1 :NumImage
        if j==1
[im_block,~, ~]=block_splitter(ImageSequence(j).Im_ict,32);
...
PSNR_still(1,k)=PSNR(1,j);
            ...
            [err_im, MV_m] = MotionEstimation(ImageSequence(j).Im_ict,ImageSequence(j-1).Im_Re,32);
            [zr1,L1]=IntraEncode_noHuffman_hevc(err_im,QP,M);
            %Build huffmancode of err_im
            pmf1=hist(zr,-500:2000);
            ...
            bytestream2=enc_huffman( MV(:)+17 , BinCode2, Codelengths2);
            L2=length(MV(:));
            ...
            MV_re_v=dec_huffman(bytestream2,BinaryTree2,L2)-17;
            MV_re=reshape(MV_re_v,h*w/(32*32),2);
            MV_m_re(:,:,1)=reshape(MV_re(:,1),h/32,w/32);
            MV_m_re(:,:,2)=reshape(MV_re(:,2),h/32,w/32);
            image_pred=MCP(ImageSequence(j-1).Im_Re,MV_m_re,32);
